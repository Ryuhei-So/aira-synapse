...
これを解決するのが RAG（Retrieval-Augmented Generation）— 論文コーパスから関連情報を検索し、それを根拠として LLM に回答させる仕組みである。しかし、学術論文に特化した RAG を作ろうとすると、汎用の Classic RAG では精度が足りない。本記事では、なぜ足りないのか、どう解決したのかを、筆者が開発した **aira-synapse** と専用グラフDB **aira-graphdb** の実装を通じて解説する。
