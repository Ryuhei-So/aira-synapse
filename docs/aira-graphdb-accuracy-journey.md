...
## Phase 14: 日本語版最適化と LLM-Acc 評価 (2026-06-23)

### JA 改善施策

英語版の改善履歴を参考に、以下の施策を日本語版に適用:

| # | 施策 | 効果 |
|---|------|------|
| 1 | GINZA tokenization (BM25 クエリ分かち書き) | 検索品質向上 |
| 2 | Entity query expansion (NER エンティティ追加) | 検索カバレッジ改善 |
| 3 | JA entity dedup (カタカナ表記揺れマージ, 8ペア) | グラフ品質向上 |
| 4 | Answer matcher JA 拡張 (カタカナ正規化、編集距離、同義語) | 評価精度向上 |
| 5 | LLM Judge 全問適用 | 意味的等価の検出 |

#### 棄却した施策（英語版の教訓）

| 施策 | 英語版での結果 | 棄却理由 |
|------|--------------|---------|
| topK 増加 (10→15/20) | **-1.3pt** | ノイズ増加で LLM 判断阻害 |
| Naive 2-hop embedding | 無効 | 89% passage 重複、ベクトル不動 |
| JA 回答強制プロンプト | **-3.5pt** | 固有名詞のカタカナ変換で誤回答増加 |
| JA 優先コンテキスト並替 | 逆効果 | 重要な英語 Fact が後方に押しやられた |

### JA ベンチマーク推移

| # | バージョン | Str-Acc | LLM-Acc | 主な変更 |
|---|-----------|---------|---------|---------|
| 1 | LadybugDB+Neo4j baseline | 58.5% | — | 基準値 |
| 2 | aira-graphdb + GINZA | 64.3% | — | GINZA 文分割 |
| 3 | v2 (4施策) | 70.8% | — | tokenize + expand + dedup + matcher |
| 4 | v3 (JA強制プロンプト) | 67.3% | — | ❌ 逆効果、棄却 |
| 5 | **v2 + LLM-Acc** | **70.8%** | **70.8%** | **LLM Judge 全問 (21問回収)** |

### JA LLM-Acc 分析

- LLM Judge 回収: 21問 / 121 calls
- Str-Acc の Answer matcher がすでに高精度のため LLM-Acc と同値
- Comparison: 83.7% (Str-Acc 80.2% から +3.5pt 改善)
- Bridge: 67.2% (LLM 非決定性による揺れ)

### Phase 15: JA Fact密度強化 & LLMモデル比較実験

**仮説**: JA の Fact 密度が EN の 12% (26.4/doc vs 226/doc) であることが精度の主因

**v3 DB 構築**: チャンクサイズ 500→1000 tokens、抽出上限 2000→4000 tokens、関係タイプ 15→23種
- 結果: 22K facts → **76K facts** (3.4倍)、Fact密度 26.4→89.4/doc

| 施策 | 精度 | v2比 | 結論 |
|------|------|------|------|
| v3 DB (76K facts) + gpt-5.4-mini | 71.0% | +0.2pt | 誤差範囲、効果なし |
| v3 DB + 英語推論プロンプト | ~65.7% | -5.1pt | 逆効果 |
| v3 DB + o4-mini | ~64.5% | -6.3pt | gpt-5.4-miniが最適 |

**結論**:
- 失敗の92%は「具体的だが誤り」（LLM推論エラー）、検索失敗は8%のみ
- 情報量の問題ではなく推論力の限界 → Fact増加は効果なし
- **v2 + gpt-5.4-mini (70.8%) がコスト・精度のベスト構成**
...
| 4 | v3 (JA強制プロンプト) | 67.3% | — | ❌ 逆効果、棄却 |
| 5 | **v2 + LLM-Acc** | **70.8%** | **70.8%** | **LLM Judge 全問 (21問回収)** |
| 6 | v3 DB (76K facts) | 71.0% | — | Fact密度3.4倍、+0.2pt（誤差） |
| 7 | v3 + o4-mini | ~64.5% | — | ❌ gpt-5.4-miniより-6.3pt |

### 論文比較（最終）

| 手法 | LLM | EN Str-Acc | EN LLM-Acc | JA Str-Acc | JA LLM-Acc |
|------|-----|-----------|-----------|-----------|-----------|
| MemGraphRAG 論文 | GPT-4o-mini | 67.2% | 71.6% | — | — |
| **我々 (aira-graphdb)** | **GPT-5.4-mini** | **89.4%** | **91.2%** | **70.8%** | **70.8%** |
| EN 差分 | — | +22.2pt | +19.6pt | — | — |

---

## 最終分析: EN vs JA ギャップの原因
...
| 失敗数 | 53 | 116 | — |
| うち推論エラー | 51 (96%) | 111 (96%) | — |
| うち検索失敗 | 2 (4%) | 5 (4%) | — |

### 根本原因

**1. Bridge問題の20.8ptギャップが支配的**

Bridge（多段推論）問題では、エンティティA → 関連情報 → エンティティB の推論チェーンが必要。
EN コーパスではエンティティ名が一貫しているが、JA コーパスでは同一エンティティが
日英混在（例: 「ブルース・ウィリス」と「Bruce Willis」）し、推論チェーンが切れる。

**2. LLM の日本語推論力の限界**

GPT-5.4-mini は英語での訓練データ量が圧倒的に多く、英語コンテキストからの推論精度が高い。
日本語の質問文解釈 → 日英混在コンテキストからの多段推論 → 正確な回答抽出の
パイプライン全体で精度が低下する。

**3. コーパス品質の差**

- EN: 英語 Wikipedia オリジナル（自然な文体、エンティティ表記が統一）
- JA: 日本語 Wikipedia（翻訳記事が多い、固有名詞の表記揺れ: ヴィ/ビ、ー/長音省略等）

**4. 評価マッチングの厳しさ**

JA は answer_ja / answer_en 両方でマッチ判定しているが、
カタカナ変換の揺れ（ヴィッカース vs ビッカース）で取りこぼしが発生。
LLM Judge でも回収不能（応答自体が別エンティティ）。
...
JA 71.0% は現アーキテクチャ（single-shot RAG + gpt-5.4-mini）の実質的上限。
ギャップの主因は「日英混在コーパスでのBridge推論チェーン断絶」であり、
これはコーパスの構造的問題とLLMの日本語推論力の限界に起因する。
アーキテクチャ変更（Fact増加、モデル変更、プロンプト改善、multi-hop）は
いずれも効果なしまたは逆効果であることを実験的に確認した。
