...
|---------|:----------------:|:------------------:|
| **MemGraphRAG（論文）** | **68.40%** | +10.80 |
| LinearRAG | 65.30% | +7.70 |
| HippoRAG2 | 65.20% | +7.60 |
| GFM-RAG | 64.10% | +6.50 |
| E2GraphRAG | 63.10% | +5.50 |
| LightRAG | 62.00% | +4.40 |
| HippoRAG | 58.40% | +0.80 |
| **aira-synapse v2** | **57.60%** | **—** |
| *aira-synapse v1* | *55.80%* | *−1.80* |
| LazyGraphRAG | 52.70% | −4.90 |
| **MS-GraphRAG** | **51.60%** | **−6.00** |

#### タスクタイプ別精度（v1 → v2 改善）

| タスクタイプ | 問題数 | v1 精度 | v2 精度 | 改善幅 |
|-------------|--------|---------|---------|--------|
| bridge（multi-hop） | 400 | 49.5% | **53.0%** | **+3.5%** |
| comparison | 100 | 81.0% | 76.0% | −5.0% |
| **合計** | **500** | **55.8%** | **57.6%** | **+1.8%** |

#### v1 → v2 改善の内訳

以下の4つの改善を適用し、特に Bridge タスクの精度向上を実現した：
...
4. **選択的 Hub 抑制**: 高次数（degree > 50）のスキーマノードのみを `1/log₂(deg+2)` で減衰。汎用的な型ノード（"person"、"city" 等）の PPR スコア支配を抑制しつつ、Comparison に必要なエンティティノードは保持

#### 分析

**MS-GraphRAG・LazyGraphRAG を大幅に上回る結果（+5〜6%）を実証。** HippoRAG に肉薄する精度。

1. **Bridge タスクの改善（+3.5%）**: Passage 優先コンテキストと QA プロンプトにより、マルチホップ推論の精度が向上。ただし、アクティブファクト率の低さ（1.9%）が依然として制約
2. **Comparison タスクの低下（-5.0%）**: 簡潔な応答スタイルにより、比較の詳細が省略されるケースが発生。v1 の長い応答は偶然一致により高スコアになっていた可能性もある
3. **コーパスカバレッジ**: 98文書中60文書でインデックスがネットワークエラーにより中断。全文書インデックス化で精度向上の余地あり
4. **論文との差（-10.8%）**: 論文実装は Python + 矛盾解決エージェントの完全実装 + 最適化されたプロンプト。aira-synapse は Stage III（矛盾解決）が簡略化されている
5. **モデルの違い**: 論文は GPT-4o-mini での評価値、aira-synapse は GPT-5.4-mini を使用
...
- **アクティブファクト率の改善**: 閾値 τ の動的調整やファクトのソフトアクティベーション（Bridge タスク 53.0% → 目標 60%+）
- **矛盾解決エージェント**: 現在のシンボリック検出に LLM ベースの解決を追加（論文との最大の差分）
- **全コーパスインデックス化**: HotpotQA の残り 38 文書を追加し、精度改善を検証
- **Comparison タスクの回復**: タスクタイプに応じたコンテキスト構築戦略の動的切替（Fact優先 vs Passage優先）
