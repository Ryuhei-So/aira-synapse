...
| 評価指標 | Str-Acc（回答内の正解含有率） |
| LLM | GPT-5.4-mini |
| PPR パラメータ | top-K=5, top-M=10, λ=0.5 |

#### 結果比較

| システム | HotpotQA Str-Acc | Δ vs aira-synapse |
|---------|:----------------:|:------------------:|
| **MemGraphRAG（論文）** | **68.40%** | +12.60 |
| LinearRAG | 65.30% | +9.50 |
| HippoRAG2 | 65.20% | +9.40 |
| GFM-RAG | 64.10% | +8.30 |
| E2GraphRAG | 63.10% | +7.30 |
| LightRAG | 62.00% | +6.20 |
| HippoRAG | 58.40% | +2.60 |
| **aira-synapse** | **55.80%** | **—** |
| LazyGraphRAG | 52.70% | −3.10 |
| **MS-GraphRAG** | **51.60%** | **−4.20** |

#### タスクタイプ別精度

| タスクタイプ | 問題数 | 正解数 | 精度 |
|-------------|--------|--------|------|
| comparison | 100 | 81 | **81.0%** |
| bridge（multi-hop） | 400 | 198 | 49.5% |
| **合計** | **500** | **279** | **55.8%** |

#### 分析

**MS-GraphRAG・LazyGraphRAG を上回る結果（+3〜4%）を実証。** ただし以下の制約に留意：

1. **コーパスカバレッジ**: 98文書中60文書でインデックスがネットワークエラーにより中断。全文書インデックス化で精度向上の余地あり
2. **Comparison タスクの高精度（81.0%）**: 2エンティティの比較は、ナレッジグラフの構造的関係の強みが発揮される。論文の知見と一致
3. **Bridge タスクの課題（49.5%）**: マルチホップ推論は、スキーマの安定化閾値やアクティブファクト率（1.9%）の保守的設定により、必要な中間事実が非アクティブ化されている可能性
4. **論文との差（-12.6%）**: 論文実装は Python + 矛盾解決エージェントの完全実装 + 最適化されたプロンプト。aira-synapse は Stage III（矛盾解決）が簡略化されている
...
- **アクティブファクト率の改善**: 閾値 τ の動的調整やファクトのソフトアクティベーション（Bridge タスク 49.5% → 目標 60%+）
