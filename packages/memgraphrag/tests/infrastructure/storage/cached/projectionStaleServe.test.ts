...
describe('CachedGraphProjection swap-on-success (literature-hub #594)', () => {
  ...
  it('fails closed when the first load fails: there is nothing to serve', async () => {
    const h = harness();
    h.state.fail = () => new OverflowError();
    h.cached.invalidateIfVersionChanged(1279);
    await expect(label(h.cached)).rejects.toMatchObject({ code: 'NATIVE_LINE_OVERFLOW' });
    // As before #594, the next call tries again (no backoff without a projection to serve).
    await expect(label(h.cached)).rejects.toMatchObject({ code: 'NATIVE_LINE_OVERFLOW' });
    expect(h.state.loads).toBe(2);
    expect(h.stale()).toEqual([]);
    expect(h.cached.servedProjection).toBeNull();
  });
  ...
  it('an explicit invalidate() drops the served projection outright', async () => {
    const h = harness();
    await label(h.cached);
    h.cached.invalidate();
    h.state.fail = () => new OverflowError();
